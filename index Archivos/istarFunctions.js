    ...
    return {
        ...
        replaceNode: function (element, typeName) {
            ...
            _.forEach(connectedLinks, function (link) {
                ...
                //     var isValid = dsm3.metamodel.nodeLinks[link.prop('type')].isValid(sourceModel, targetModel);
                //     console.log(isValid);
                // }
            });
        ...
        addLinkBetweenActors: function (linkType, source, target) {
            var hasShape = dsm3.metamodel.shapesObject[linkType.name];
            if (source.source) {
        ...
        addDependency: function (depender, dependum, dependee) {
            var shape = joint.shapes.dsm3.DependencyLink;
            var hasShape = true;
            if (!shape) {
                var shape = joint.shapes.dsm3.DependencyLink || joint.shapes.dsm3.DefaultContainerLink;
                hasShape = false;
            }
            var link1 = new shape({
        ...
        addLinkBetweenNodes: function (linkType, source, target, value) {
            if (linkType.sourceMultiplicity === -1) {
                return this.addSplitLink(linkType, source, target, value);
            }
            else {
                if (source.source) {
                    return this.addLinkBetweenNodesWithOptions(linkType, source, target);
                }
                if (linkType.targetsLink) {
                    var targetLinkView = target.findView(istar.paper);
                    var midPoint = g.Line(targetLinkView.sourcePoint, targetLinkView.targetPoint).midpoint();
                    var myTarget = midPoint;//new g.Point(228, 100);
                } else {
                    var myTarget = {id: target.id};
                }
                var hasShape = dsm3.metamodel.shapesObject[linkType.name];
                var myOptions = {'source': {id: source.id}, 'target': myTarget};
                var link = new linkType.shapeObject(myOptions);
                link.prop('type', linkType.name);
                istar.graph.addCell(link);

                if (linkType.targetsLink) {
                    link.prop('virtualTarget', target);
                }
                if (linkType.label) {
                    link.attr('label/text', linkType.label);
                    link.attr('label-background/text', linkType.label);
                ...
                if (linkType.customProperties) {
                    link.prop('customProperties', linkType.customProperties);
                }
                if (linkType.isSplitLink) {
                    link.prop('isSplitLink', true);
                }

                return link;
            }
        },
        addLinkBetweenNodesWithOptions: function (linkType, options, value) {
            if (linkType.targetsLink) {
                var targetLinkView = options.target.findView(istar.paper);
                var midPoint = g.Line(targetLinkView.sourcePoint, targetLinkView.targetPoint).midpoint();
                options.target = midPoint;//new g.Point(228, 100);
            }
            else {
                options.target = {id: options.target.id};
            }
            var hasShape = dsm3.metamodel.shapesObject[linkType.name];
            var link = new linkType.shapeObject(options);
            link.prop('type', linkType.name);
            istar.graph.addCell(link);

            if (linkType.targetsLink) {
                link.prop('virtualTarget', options.target);
            }
            if (linkType.label) {
                link.attr('label/text', linkType.label);
                link.attr('label-background/text', linkType.label);
            ...
            //embeds the link on the (parent) actor of its source element, to facilitate collapse/expand
            if (options.source.get('parent')) {
                istar.graph.getCell(options.source.get('parent')).embed(link);
            }
        ...
        addSplitLink: function (linkType, source, target, value) {
            if (source.source) {
                return this.addLinkBetweenNodesWithOptions(linkType, source, target);
            }
            if (linkType.targetsLink) {
            ...
            var sourceElement = istar.graph.getCell(myOptions.source.id);
            var targetElement = istar.graph.getCell(myOptions.target.id);
            var linkCenterX = (sourceElement.getBBox().x + sourceElement.getBBox().width/2 + targetElement.getBBox().x + targetElement.getBBox().width/2)/2
            var linkCenterY = (sourceElement.getBBox().y + targetElement.getBBox().y)/2
            var baseName = linkType.name;
            var hub = new joint.shapes.dsm3[baseName + 'Hub']();
            hub.position(linkCenterX, linkCenterY);
            var startTarget = hub;
            var link = new joint.shapes.dsm3[baseName + 'Start']({source: myOptions.source, target: hub});
            var linkEnd = new joint.shapes.dsm3[baseName + 'End']({source: hub, target: myOptions.target});
            // link.prop('type', linkType.name);
            istar.graph.addCell(hub);
            istar.graph.addCell(link);
            istar.graph.addCell(linkEnd);
            hub.prop('isSplitLink', true);
            link.prop('isSplitLink', true);
            linkEnd.prop('isSplitLink', true);
            ...
            if (linkType.customProperties) {
                hub.prop('customProperties', linkType.customProperties);
            }
