...
istar.setupMetamodel = function (metamodel) {
    ...
    function attachShapeObject(cellType, metamodel, kindOfCell) {
        ...
        if (cellType.name) {
            ...
            if (!cellType.shapeObject) {
                ...
                if (kindOfCell === 'node') {
                    cellType.shapeObject = joint.shapes.dsm3.DefaultNode;
                }
                else if (kindOfCell === 'container') {
                    cellType.shapeObject = joint.shapes.dsm3.DefaultContainer;
                }
                else if (kindOfCell === 'containerLink') {
                    cellType.shapeObject = joint.shapes.dsm3.DefaultContainerLink;
                }
                else {
                    cellType.shapeObject = joint.shapes.dsm3.DefaultNodeLink;
                }
            }
        }
    }
    ...
    function createAddContainerLinkFunction (linkType) {
        istar['add' + linkType.name] = function (source, target) {
            // if (dsm3.metamodel.containerLinks[linkType.name].isValid(source, target)) {
    ...
    function createAddNodeLinkFunction (linkType) {
        istar['add' + linkType.name] = function (source, target, label) {
            // if (dsm3.metamodel.nodeLinks[linkType.name].isValid(source, target)) {
    ...
    function setupNodeAttributesDefaultValues (elementType) {
        // if (elementType.canBeInnerElement === undefined) {
        //     elementType.canBeInnerElement = true;
        // }
        // if (elementType.canBeDependum === undefined) {
        //     elementType.canBeDependum = true;
        // }
    ...
    function setupSplitLinks (metamodel) {
        _.forEach(metamodel.nodeLinks, function (linkType) {
            if (linkType.sourceMultiplicity === -1) {
                metamodel.nodes[linkType.name + 'Hub'] = {
                    'name': linkType.name + 'Hub',
                    'canBeDependum': false,
                    'canBeInnerElement': false,
                    'canBeOnPaper': false,
                    'isSplitLink': true
                };
                var isValid = metamodel.nodeLinks[linkType.name + 'Start'].isValid;
                metamodel.nodeLinks[linkType.name + 'Start'] = {
                    'name': linkType.name + 'Start',
                    'isSplitLink': true
                };
                if (isValid) {
                    metamodel.nodeLinks[linkType.name + 'Start'].isValid = isValid;
                }
                metamodel.nodeLinks[linkType.name + 'End'] = {
                    'name': linkType.name + 'End',
                    'isSplitLink': true,
                    'canBeManuallyAdded': false
                };
            }
        });
