        ...
        saveModel: function () {
            ...
            _.forEach(istar.graph.getElements(), function (element) {
                if (element.isKindOfActor()) {
                    ...
                    if (element.prop('isSplitLink')) {
                        var JSONLinkHub = elementToJSON(element);
                        ...
                        var linkStarts = istar.graph.getConnectedLinks(element, {inbound: true});
                        var JSONLinkStarts = [];
                        _.forEach(linkStarts, function(link) {
                            JSONLinkStarts.push(linkToJSON(link));
                        });
                        var JSONLinkEnd = linkToJSON(istar.graph.getConnectedLinks(element, {outbound: true})[0]);


                        modelJSON.links.push({
                            hub: JSONLinkHub,
                            starts: JSONLinkStarts,
                            end: JSONLinkEnd
                        });
                    }
                    else {
            ...
            function elementToJSON (element) {
                ...
                var result = {
                    'id': element.id,
                    'text': text,
                    'type': dsm3.metamodel.prefix + '.' + element.prop('type'),
            ...
            function linkToJSON (link) {
                var myTarget = link.attributes.target.id || link.attributes.virtualTarget.id;
                var result = {
                    id: link.id,
                    type: dsm3.metamodel.prefix + '.' + link.prop('type'),
            ...
            if (inputRaw) {
                ...
                if (inputModel.actors) {
                    ...
                    for (i = 0; i < inputModel.dependencies.length; i++) {
                        ...
                        var isValid = dsm3.metamodel.dependencyLinks['DependencyLink'].isValid(depender, dependee, (dependum.prop('type') + 'DependencyLink'));
                        if (!isValid.isValid) {
                    ...
                    for (i = 0; i < inputModel.links.length; i++) {
                        ...
                        if (! isDependencyLink(linkJSON)) {
                            if (linkJSON.hub) {
                                var newElement = addLoadedElement(linkJSON.hub, inputModel.display);
                                newElement.prop('isSplitLink', true);

                                var newLink = addLoadedLink(linkJSON.end);
                                if (inputModel.display && inputModel.display[linkJSON.id] && inputModel.display[linkJSON.id].vertices) {
                                    newLink.set('vertices', inputModel.display[linkJSON.id].vertices);
                                }
                                newLink.prop('isSplitLink', true);

                                _.forEach(linkJSON.starts, function(link) {
                                    var newLink = addLoadedLink(link);
                                    if (inputModel.display && inputModel.display[link.id] && inputModel.display[link.id].vertices) {
                                        newLink.set('vertices', inputModel.display[link.id].vertices);
                                    }
                                    newLink.prop('isSplitLink', true);
                                });
                            }
            ...
            function addLoadedLink (linkJSON) {
                if (linkJSON.id && linkJSON.type && linkJSON.source && linkJSON.target) {
                    ...
                    if (istar['add' + typeNameWithoutPrefix]) {
                        ...
                        if (_.includes(dsm3.metamodel.getNodeLinksNames(), typeNameWithoutPrefix)) {
                            var isValid = dsm3.metamodel.nodeLinks[typeNameWithoutPrefix].isValid(sourceObject, targetObject);
                            if (!isValid.isValid) {
                                processInvalidLink(typeNameWithoutPrefix, sourceObject, targetObject, isValid);
                            }
                        } else if (_.includes(dsm3.metamodel.getContainerLinksNames(), typeNameWithoutPrefix)) {
                            var isValid = dsm3.metamodel.containerLinks[typeNameWithoutPrefix].isValid(sourceObject, targetObject);
                            if (!isValid.isValid) {
                                processInvalidLink(typeNameWithoutPrefix, sourceObject, targetObject, isValid);
                            }
                        }

                        var newLink = istar['add' + typeNameWithoutPrefix](options, linkJSON.label);
                        ...
                        if (dsm3.metamodel.nodeLinks[typeNameWithoutPrefix]) {
                            shapeObject = new dsm3.metamodel.nodeLinks[typeNameWithoutPrefix].shapeObject();
                        }
                        else {
                            shapeObject = new dsm3.metamodel.containerLinks[typeNameWithoutPrefix].shapeObject();
                        }
