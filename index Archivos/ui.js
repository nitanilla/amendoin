...
ui.defineInteractions = function () {
    ...
    istar.paper.on('blank:pointerdown', function (evt, x, y) {
        ...
        if (ui.states.editor.isAddingNode()) {
            ...
            var nodes = _.keys(dsm3.metamodel.nodes);
            var bbox = (new dsm3.metamodel.nodes[nodes[0]].shapeObject()).getBBox();
    ...
    istar.paper.on('cell:pointerup', function (cellView, evt, x, y) {
        ...
        if (ui.states.editor.isAddingNode()) {
            ...
            var isContainerLink = _.includes(dsm3.metamodel.getContainerLinksNames(), currentAddingElement);
            var isNodeLink = _.includes(dsm3.metamodel.getNodeLinksNames(), currentAddingElement);
            ...
            if (isContainerLink) {
                if (cellView.model.isKindOfActor()) {
                    if (ui.states.editor.ADDING.data.isLinkSourceUndefined()) {
                        ...
                        var isValid = dsm3.metamodel.containerLinks[currentAddingElement].isValid(ui.states.editor.ADDING.data.linkSourceView.model, ui.states.editor.ADDING.data.linkTargetView.model);
                        if (isValid.isValid) {
                ...
                if (ui.states.editor.ADDING.data.isLinkSourceUndefined()) {
                    ...
                    if (isDependencyLink) {
                        var isValid = dsm3.metamodel.dependencyLinks['DependencyLink'].isValid(ui.states.editor.ADDING.data.linkSourceView.model, ui.states.editor.ADDING.data.linkTargetView.model);
                        if (isValid.isValid) {
                    ...
                    if (isNodeLink) {
                        var newLink = null;
                        var prettyLinkName = '';
                        var isValid = dsm3.metamodel.nodeLinks[currentAddingElement].isValid(ui.states.editor.ADDING.data.linkSourceView.model, ui.states.editor.ADDING.data.linkTargetView.model);

                        if (dsm3.metamodel.nodeLinks[currentAddingElement].tryReversedWhenAdding) {
                            if (!isValid.isValid) {
                                //try with reversed source/targets
                                var isValidReversed = dsm3.metamodel.nodeLinks[currentAddingElement].isValid(ui.states.editor.ADDING.data.linkTargetView.model, ui.states.editor.ADDING.data.linkSourceView.model);
                                if (isValidReversed) {
                        ...
                        if (isValid.isValid) {
                            ...
                            if (dsm3.metamodel.nodeLinks[currentAddingElement].changeableLabel) {
...
ui.addElementOnPaper = function (options) {
    ...
    try {
        ...
        if (ui.states.editor.isAddingNode()) {
            if (dsm3.metamodel.nodes[currentAddingElement]) {
                if (dsm3.metamodel.nodes[currentAddingElement].canBeOnPaper) {
                    isValid = dsm3.metamodel.nodes[currentAddingElement].isValid();
                }
                else {
                    isValid = {
                        message: 'a ' + currentAddingElement + ' cannot be added directly to the paper, it must be added <b>inside</b> an Actor.'
                    };
                    if (dsm3.metamodel.nodes[currentAddingElement].canBeDependum) {
            ...
            if (dsm3.metamodel.containers[currentAddingElement]) {
                isValid = dsm3.metamodel.containers[currentAddingElement].isValid();
            }
        }

        if (isValid.isValid) {
            var newActor = istar['add' + currentAddingElement]('', options);
            if (dsm3.metamodel.nodes[currentAddingElement] && dsm3.metamodel.nodes[currentAddingElement].customProperties) {
                newActor.prop('customProperties', dsm3.metamodel.nodes[currentAddingElement].customProperties);
            }
            else if (dsm3.metamodel.containers[currentAddingElement] && dsm3.metamodel.containers[currentAddingElement].customProperties) {
                newActor.prop('customProperties', dsm3.metamodel.containers[currentAddingElement].customProperties);
            }
...
ui.addElementOnContainer = function (cellView, options) {
    ...
    try {
        ...
        var isValid = {isValid: false};
        if (dsm3.metamodel.nodes[currentAddingElement]) {
            if (dsm3.metamodel.nodes[currentAddingElement].canBeInnerElement) {
                isValid = dsm3.metamodel.nodes[currentAddingElement].isValid(cellView.model);
            }
            else {
                isValid = {
        ...
        if (isValid.isValid) {
            ...
            var bbox = (new dsm3.metamodel.nodes[currentAddingElement].shapeObject()).getBBox();
            ...
            if (dsm3.metamodel.nodes[currentAddingElement].customProperties) {
                element.prop('customProperties', dsm3.metamodel.nodes[currentAddingElement].customProperties);
            }
...
ui.addLinkBetweenContainers = function (newLink, targetCellView) {
    ...
    try {
        ...
        if (dsm3.metamodel.containerLinks[newLink].isValid(ui.states.editor.ADDING.data.linkSourceView.model, ui.states.editor.ADDING.data.linkTargetView.model)) {
...
ui.displayInvalidModelMessage = function (messages) {
    ...
    if (messages) {
        var text = '<div class="alert alert-danger">Hello there! The model you are loading has some invalid elements or links. ' +
            'Please address the issues listed below ' +
